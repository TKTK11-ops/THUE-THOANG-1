  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
        <div className="grid grid-cols-1 md:grid-cols-[minmax(0,4fr)_minmax(0,8fr)]">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex items-baseline gap-1.5">
                {property.pricePrefix && (
                  <span className="text-sm text-neutral-500">{property.pricePrefix}</span>
                )}
                <span className="text-xl font-medium text-[#468cc8]">
              ...
              <div
                className="flex items-baseline gap-1.5 text-[#468cc8] ml-0 md:ml-2"
                title="Distance from search location"
              >
                <MapPin size={14} className="relative top-[1px]" />
                <span className="text-xl font-medium">{property.distanceKm.toFixed(2)}</span>
                <span className="text-sm text-neutral-500">km</span>
              </div>

              <div className="hidden lg:flex items-center gap-1.5 text-neutral-400 ml-auto text-sm">
                <Clock size={14} />
                <span>{property.lastUpdated}</span>
              </div>
            </div>

            <div className="font-medium text-[#468cc8] text-lg md:text-xl">
              {property.title}
            </div>

            <div className="hidden md:block">
              <p className="text-neutral-600 text-sm leading-relaxed line-clamp-2 m-0">
                {property.description}
              </p>
            </div>

            <div className="flex gap-4 justify-between items-end mt-auto">
              <ul className="flex flex-wrap text-xs md:text-sm font-medium text-neutral-700 list-none p-0 m-0 gap-x-0">
              ...
              <div className="hidden md:block shrink-0">
                <span className="inline-flex items-center px-5 py-2.5 rounded-lg bg-[#198754] text-white text-sm font-medium hover:bg-[#157347] transition-colors">
                  View Details
                </span>
              </div>
            </div>
